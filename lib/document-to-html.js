...
function DocumentConversion(options, comments) {
    ...
    function convertStyleForAttributes(element, options) {
        ...
        if (element.font) {
            styleMap.push({key: 'font-family', value: element.font.cs || element.font.eastAsia || '仿宋'});
        }
        if (element.fontSize) {
    ...
    function createBgColor(color) {
        return color === 'auto' || !color ? '#FFFFFF' : ('#' + color);
    }
    ...
    function convertTable(element, messages, options) {
        ...
        if (element.columnWidths) {
            optionResult['data-columnWidths'] = JSON.stringify(element.columnWidths);
        }
        if (element.float) {
        ...
        if (element.cellMargin) {
            optionResult['data-cellMargin'] = JSON.stringify(element.cellMargin);
        }
        if (element.overlap) {
    ...
    function convertTableCell(element, messages, options) {
        ...
        if (element.verticalMerge) {
            attributes['data-verticalMerge'] = JSON.stringify(element.verticalMerge);
        }
        if (element.verticalAlign) {
            attributes['data-verticalAlign'] = element.verticalAlign;
        }
        if (element.margins) {
    ...
    function convertCustomDocDesc(element, messages, options) {
        console.log(element, messages, options, 'convertCustomDocDesc');
        return [];
    }
